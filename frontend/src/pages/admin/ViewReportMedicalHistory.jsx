...
const ViewReportMedicalHistory = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await adminService.getPatientMedicalHistory(email.trim());
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sm:p-8">
        <div className="mb-6">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900">View Medical History</h2>
          <p className="text-gray-500 text-sm mt-1">Search for a patient by their email to view medical history</p>
        </div>
        ...
        {medicalHistory && (
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="bg-[#007a8a] px-6 py-4">
              ...
              <p className="text-white/80 text-sm">Last updated: {formatDate(medicalHistory.updatedAt)}</p>
            </div>
            ...
            <div className="p-6 space-y-6">
              {medicalHistory.diagnosis && (
                <div>
                  ...
                  <p className="text-gray-900 bg-gray-50 rounded-lg p-4">{medicalHistory.diagnosis}</p>
                </div>
              )}

              {medicalHistory.medications && (
                <div>
                  <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-2">Medications</h4>
                  <p className="text-gray-900 bg-gray-50 rounded-lg p-4">{medicalHistory.medications}</p>
                </div>
              )}
              ...
              {medicalHistory.allergies && (
                <div>
                  ...
                  <p className="text-gray-900 bg-gray-50 rounded-lg p-4">{medicalHistory.allergies}</p>
                </div>
              )}

              {medicalHistory.notes && (
                <div>
                  <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-2">Doctor Notes</h4>
                  <p className="text-gray-900 bg-gray-50 rounded-lg p-4">{medicalHistory.notes}</p>
                </div>
              )}

              {!medicalHistory.diagnosis && !medicalHistory.medications && !medicalHistory.allergies && !medicalHistory.notes && (
